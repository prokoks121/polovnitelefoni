...
function SendKontakt()
{
    ...
    if (count($errors) == 0)
    {
      $ip = getIps();
        ...
        $stm = $conns->prepare("INSERT INTO kontakt (`email`,`ime`,`txt`,`ip`) VALUES (?,?,?,?)");
        $stm->execute([$email, $ime, $txt,$ip]);
...
function followUser()
{
    ...
    if (isset($_SESSION['user']['id']))
    {
        ...
        if ($num == 0)
        {
            ...
            $stm = $conns->prepare("INSERT INTO follow (`id_user`, `folowed_user`,`ip`) VALUES (?, ?,?)");
            $stm->execute([$followed_user, $user_id,$ip]);
